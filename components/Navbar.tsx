...
const Navbar: React.FC<NavbarProps> = ({ activeSection, scrollToSection }) => {
  ...
  return (
    ...
    >
          ...
          {navLinks.map((link) => (
            <button
              ...
              className={`text-sm font-medium tracking-wide uppercase transition-all duration-300 relative group ${
                activeSection === link.id ? 'text-neon-cyan' : 'text-slate-400 hover:text-white'
              }`}
            >
              {link.label}
              <span className={`absolute -bottom-1 left-0 h-[2px] bg-neon-cyan transition-all duration-300 ${
      ...
      {/* Mobile Menu */}
      <div className={`fixed inset-0 bg-void/95 backdrop-blur-xl z-30 transform transition-transform duration-500 flex flex-col items-center justify-center gap-8 ${
